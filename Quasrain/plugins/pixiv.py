...
async def pixiv(session: CommandSession):
    ...
    city = session.get('tag', prompt='你想查询哪个城市的天气呢？')
    # 获取城市的天气预报
    weather_report = await get_url_of_tag(city)
    # 向用户发送天气预报
...
async def get_url_of_tag(tag: str) -> str:
    # 这里简单返回一个字符串
    ...
    for urls in result:
        ...
        # print(trueurl+Res[0]+'\n')
        print(trueurl+urls+'\n')
        ans = ans+"[CQ:image,file="+trueurl+urls+"]"+'\n'
